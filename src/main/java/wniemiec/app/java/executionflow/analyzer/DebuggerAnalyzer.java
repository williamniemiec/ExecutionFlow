...
import wniemiec.app.java.executionflow.analyzer.exceptions.AssertFailureException;
...
public abstract class DebuggerAnalyzer {
	...
	private void disableTimeout(long timeoutId) {
	...
	private List<Path> getSourcePath(Invoked invokedInfo, 
		...
		Path mavenSrcPath = ExecutionFlow.getCurrentProjectRoot().resolve(
				Path.of("src", "main", "java")
		);
		srcPath.add(mavenSrcPath);
		
		Path mavenTestPath = ExecutionFlow.getCurrentProjectRoot().resolve(
	 ...
	 * wniemiec.app.java.executionflow.runtime.CollectCalls}' will be considered 
	 */
