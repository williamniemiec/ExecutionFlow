...
//		System.out.println(bracketCount);
		...
		if (i+1 < strLine.length()){
			bracketCount = parseWithComments(strLine, bracketCount, i);
		}
		else {
			bracketCount = parseWithoutComments(strLine, bracketCount);
		}
		...
		if (!commentVar.isBlank())
			System.out.println(commentVar);
		else
			System.out.println(javaVar);
	}

	private int parseWithoutComments(String strLine, int bracketCount) {
		String strIndent;
		// Comment impossible
		javaVar = strLine;
	...
	private int parseWithComments(String strLine, int bracketCount, int i) {
		String strIndent;
