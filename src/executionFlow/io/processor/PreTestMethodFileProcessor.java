...
public class PreTestMethodFileProcessor extends FileProcessor
{
	...
	private void doProcessing(List<String> lines) {
		AssertProcessor assertProcessor = new AssertProcessor();
		assertProcessor.processLines(lines);
		
		
	}
	
	private void doProcessingOld(List<String> lines) {
		currentLine = 1;
...
//		AssertParser assertParser = new AssertParser();
		...
		for (int i=0; i<lines.size(); i++) {
			...
			boolean belongsToThePreviousLineVariable = previousLine.matches(".+=[\\s\\t]*$");
			
			// Checks if it is multiline comment
			checkComments(line);
			
			// Process the line
			if (!insideMultilineComment && !belongsToThePreviousLineVariable) {
	...
	private class AssertParser
	{
		...
		//		Attributes
		//---------------------------------------------------------------------
		private final RoundBracketBalance roundBracketsBalance = new RoundBracketBalance();
		...
		{
			if (inAssert)
				line = processMultilineAssert(line);
			else if (isAssertInstruction(line))
				line = processAssert(line);
