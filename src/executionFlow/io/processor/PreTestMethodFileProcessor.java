...
public class PreTestMethodFileProcessor extends FileProcessor
{
	...
	public String processFile() throws IOException
	{
		...
		if (ConsoleOutput.getLevel() == ConsoleOutput.Level.DEBUG) {
			ConsoleOutput.showDebug("PreTestMethodFileProcessor", "Processed file");
	...
	private class AnnotationParser
	{
		...
		private String parseParameterizedTest(String line)
		{
			...
			// Gets enum type
				...
				if (m.find()) {
					param_enumType = line.split(m.group())[1].split("\\.")[0].trim();
				}
				// EnumSource without value attribute but with class specified
				else if (line.contains("(")) {
					param_enumType = line.substring(line.indexOf("(")+1, line.indexOf(")")).split("\\.")[0].trim();
				}
				// Empty EnumSource 
				else {
					param_enumType = "";
